...
def bill_gui_args(invoice_gui, cust_name_gui):
    for x in invoice_gui:
        print x, invoice_gui[x], invoice_gui[x][0], invoice_gui[x][2]

    connection_db = sqlite3.connect(D_BASE_URL)
    csr_gui = connection_db.cursor()
    b_number = random.randrange(1, 99999)
    print b_number
    csr_gui.execute("insert into invoice(customer_id, invoice_number) values (?,?)", (1, b_number))
    for x in invoice_gui:
        csr_gui.execute('''insert into invoice_items(invoice_id, item_id, quantity) values (?, ?, ?)''',
                        (b_number, int(invoice_gui[x][0]), float(invoice_gui[x][2])))
